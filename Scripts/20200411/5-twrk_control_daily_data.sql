...
from Vew_EmployeeInfo e
		left join (select * from TblDailyRecord r 
					...
					where SystemDate='2019-08-15' ) dr  --where SystemDate=@dt01 between @FromDate and @ToDate
		on e.EmployeeID=dr.EmployeeID
where (case when 20190724232145633=11111 then 'Y' 
		when e.TeamID= 20190724232145633  then 'Y' 
		else 'N'
	   end)='Y'
and (case when '' is null or ''='' then 'Y' 
			when upper(e.EmployeeCardNo)= ''  then 'Y' 
			else 'N'
		   end)='Y'
and e.DateJoined <='2019-08-15' 
and e.RecStatus='A'
